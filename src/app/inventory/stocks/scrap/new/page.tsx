...
const initialItems: ScrapLineItem[] = [
  {
    id: "1",
    product: "",
    product_description: "",
...
export default function Page() {
  ...
  // Form state
  const [items, setItems] = useState<ScrapLineItem[]>(initialItems);
  ...
  const breadcrumsItem: BreadcrumbItem[] = [
    ...
    { label: "New", href: "/inventory/stocks/scrap/new", current: true },
  ];
  ...
  async function onSave(data: ScrapFormData): Promise<void> {
    ...
    try {
      await createScrap(scrapData).unwrap();

      setNotification({
        message: "Scrap record saved as draft!",
        type: "success",
        show: true,
      });
  ...
  return (
    ...
    >
      ...
      >
        <PageHeader items={breadcrumsItem} title="New Scrap Record" />
      </motion.div>
